	...
	.globl	fib
fib:
	addi	sp, sp, -48	# Allocate a 16-byte-aligned 48-byte frame
	sd	ra, 40(sp)	# saves returnAddress SlotReturnAddress; Save returnAddress: every Call clobbers it
	sd	s0, 32(sp)	# saves s0 SlotSavedS0; Preserve caller's s0 before we reuse it for number
	sd	s1, 24(sp)	# saves s1 SlotSavedS1; Preserve caller's s1 before we reuse it for fib(number-1)
	...
	add	a0, s1, a0	# reads firstResult; writes result; result = fib(number-1) + fib(number-2)
.Lepilogue:
	ld	ra, 40(sp)	# restores returnAddress SlotReturnAddress; Restore return address
	ld	s0, 32(sp)	# restores s0 SlotSavedS0; Restore caller's s0 (callee-saved contract)
	ld	s1, 24(sp)	# restores s1 SlotSavedS1; Restore caller's s1 (callee-saved contract)
