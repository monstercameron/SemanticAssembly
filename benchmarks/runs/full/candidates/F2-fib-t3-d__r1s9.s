...
fib:
	...
	sd	a0, 0(sp)	# writes firstResult; spill fib(number-1) to the stack before the second Call clobbers a0
	addi	a0, s0, -2	# reads number; argument = number - 2
	call	fib	# a0 = fib(number-2). firstResult survives in the stack slot — safe; number (s0) is now dead
	ld	t2, 0(sp)	# reads firstResult; reload fib(number-1) from the stack
