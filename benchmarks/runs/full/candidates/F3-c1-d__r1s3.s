...
fib:
	...
	call	fib	# liveOut s1:number; a0 = fib(number-1). number survives in s0 (callee-saved) — safe across the call
	mv	t1, s0	# reads number; writes number; copy number into t1 right after the first call returns
	mv	s1, a0	# writes firstResult; fib(number-1) -> s1 before the second Call clobbers a0
	addi	a0, t1, -2	# reads number; argument = number - 2 (computed from t1)
