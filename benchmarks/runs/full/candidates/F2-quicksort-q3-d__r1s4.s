	...
	.globl	qsort64
qsort64:
	addi	sp, sp, -48	# Allocate the 16-byte-aligned 48-byte frame
	...
	sd	s3, 32(sp)	# saves s3 SlotSavedS3; Preserve caller's s3 before it carries low
	mv	s3, a1	# reads low; low -> s3: carry low in callee-saved s3 for the whole function
	...
	addi	t3, s3, -1	# reads low; writes boundary; boundary = low - 1 (empty prefix)
	mv	t4, s3	# reads low; writes scan; scan = low
.Lpartitioncheck:
	...
	blt	t2, t5, .Ladvancescan	# reads pivotValue; pivotValue < element: element stays in the > suffix
...
.Lpartitiondone:
	...
	addi	a2, t0, -1	# reads pivotIndex; left recursion bound: pivotIndex - 1
	mv	a1, s3	# reads low; set a1 from s3 immediately before the first recursive call
	call	qsort64	# liveOut s0:array; liveOut s1:high; liveOut s2:pivotIndex; liveOut s3:low; qsort64(array, low, pivotIndex-1); values ride out the call in s0/s1/s2/s3
...
.Lepilogue:
	...
	ld	s3, 32(sp)	# restores s3 SlotSavedS3; Restore caller's s3
	addi	sp, sp, 48	# Free the frame
